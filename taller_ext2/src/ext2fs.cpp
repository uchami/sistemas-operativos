...
struct Ext2FSInode * Ext2FS::load_inode(unsigned int inode_number){
	unsigned int blockLogSize =  _superblock->log_block_size;
	unsigned int blockSize = 1024<<blockLogSize;
	Ext2FSInode* inode = (struct Ext2FSInode *) malloc(sizeof(Ext2FSInode));
	unsigned int inodeSize = _superblock->inode_size;
	unsigned int inodeIndex = blockgroup_inode_index(inode_number);


	unsigned int blockNumber = blockgroup_for_inode(inode_number);
	Ext2FSBlockGroupDescriptor* blockDescrpitor = block_group(blockNumber);
	unsigned int blockAddress = blockDescrpitor->inode_table + (inodeIndex / (blockSize/inodeSize));
	 

	int offset = inodeIndex % (blockSize/inodeSize);

	unsigned char buffer[blockSize];

	read_block(blockAddress,buffer);

	Ext2FSInode *bufferTable = (Ext2FSInode *)buffer;

	*inode = bufferTable[offset];
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	unsigned int blockLogSize =  _superblock->log_block_size;
	unsigned int blockSize = 1024<<blockLogSize;
	unsigned int blockAmount = blockSize/sizeof(int);
	unsigned int single = 12 + blockAmount;
	...
	}else if(block_number < single){
		int lba = inode->block[12];
		read_block(lba,buffer);
		return ((unsigned int*)buffer)[block_number - single];
		
	}else if(block_number < single + blockAmount*blockAmount){
		int lba = inode->block[13];
		read_block(lba,buffer);
		
		return ((unsigned int*)buffer)[block_number - (single + blockAmount*blockAmount)];
	
	}else if(block_number < single + blockAmount*blockAmount + blockAmount*blockAmount*blockAmount ){
		int lba = inode->block[14];
		read_block(lba,buffer);
		
		return ((unsigned int*)buffer)[block_number - (single + blockAmount*blockAmount + blockAmount*blockAmount*blockAmount)];
...
{	
	...
	while(true){
		...
		if(offset == 0 || offset > blockSize) {
			blockAddress = get_block_address(from, currentBlock);
			read_block(blockAddress,buffer);
			currentBlock++;
			offset = 0;
		}
		
		Ext2FSDirEntry* dirEntry = (Ext2FSDirEntry*) (buffer + offset);
		
		if(strcmp(dirEntry->name, filename)){
			return load_inode(dirEntry->inode);
		}

		offset += dirEntry->record_length;
	}
}
